...
class RobotPoseController:
    ...
    def __init__(self, host, rtde_port=30004, script_port=30002):
        ...
        try:
            ...
            # Setup output recipe
            if not self.rtde.send_output_setup(output_names, output_types, frequency=125):
    ...
    def _wait_for_move(self, timeout=10):
        ...
        while time.time() - start_time < timeout:
            ...
            if np.linalg.norm(np.array(current_pose[:3]) - np.array(initial_pose[:3])) < 0.001:
                # Check velocity
                self._update_state()
...
def explain_joint_positions():
    ...
    print("  - Use POSE when: You care about where the tool is, not how the arm gets there")
...
def get_pose(robot, filename, pose_name="current", force=False, explain=False):
    ...
    print(f"  Joints 1-6: Base, Shoulder, Elbow, Wrist1, Wrist2, Wrist3 (angles in radians)")
    ...
    if pose_name in poses and not force:
        response = input(f"Pose '{pose_name}' already exists in {filename}. Overwrite? [y/N]: ")
...
def set_pose(robot, filename=None, pose_name=None, pose_str=None, acc=0.3, vel=0.05, prefer_joints=False):
    ...
    if pose_str:
        ...
        if prefer_joints and has_joints:
            print(f"Moving to joint positions for '{pose_name}': {format_joints(pose_data['joints'])}")
            ...
            if has_joints:
                print(f"  (Note: Joint data also available, but using pose. Use --joints to use joint positions instead)")
...
def main():
    ...
    parser.add_argument("-H", "--host", default=ROBOT_HOST,
                       help=f"Robot hostname or IP (default: from UR_HOST env or {ROBOT_HOST})")
    parser.add_argument("-f", "--file", default=DEFAULT_POSE_FILE,
                       help=f"Pose file (default: {DEFAULT_POSE_FILE})")
    
    # Action arguments
    action_group = parser.add_mutually_exclusive_group(required=True)
    action_group.add_argument("-g", "--get", action="store_true",
                             help="Get current pose and save to file")
    action_group.add_argument("-s", "--set", action="store_true",
                             help="Move to a pose")
    action_group.add_argument("-l", "--list", action="store_true",
                             help="List all poses in file")
    
    # Pose specification arguments
    parser.add_argument("-n", "--pose-name", default="current",
                       help="Name for pose when getting, or name to use when setting (default: 'current')")
    parser.add_argument("-p", "--pose",
                       help="Pose as comma-separated values: 'x, y, z, rx, ry, rz'")
    
    # Movement parameters
    parser.add_argument("-a", "--acc", type=float, default=0.3,
                       help="Acceleration (default: 0.3)")
    parser.add_argument("-v", "--vel", type=float, default=0.05,
                       help="Velocity (default: 0.05)")
    ...
    parser.add_argument("-j", "--joints", action="store_true",
    ...
    try:
        if args.get:
            ...
            success = set_pose(robot, args.file, args.pose_name, args.pose, args.acc, args.vel, args.joints)
            if not success:
                sys.exit(1)
    finally:
        robot.close()
